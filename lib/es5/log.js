...
var Log = function (_GSet) {
   ...
   * @return {Log}                            Log
   */
  function Log(ipfs, id, entries, heads, clock, validator) {
    ...
    // Entry validator
    _this._entryValidator = new EntryValidator(validator);
  ...
  (0, _createClass3.default)(Log, [{
    ...
    value: function fromMultihash(ipfs, hash) {
      ...
      return LogIO.fromMultihash(ipfs, hash, length, exclude, onProgressCallback).then(function (data) {
        return new Log(ipfs, data.id, data.values, data.heads, data.clock, entryValidator);
      });
    }
    ...
    value: function fromEntryHash(ipfs, hash, id) {
      ...
      return LogIO.fromEntryHash(ipfs, hash, id, length, exclude, onProgressCallback).then(function (data) {
        return new Log(ipfs, id, data.values, null, null, entryValidator);
      });
    }
    ...
    value: function fromJSON(ipfs, json) {
      ...
      return LogIO.fromJSON(ipfs, json, length, timeout, onProgressCallback).then(function (data) {
        return new Log(ipfs, data.id, data.values, null, null, entryValidator);
      });
    }
