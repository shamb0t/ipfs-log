  ...
  describe('Log', function() {
    ...
    describe('constructor', async () => {
      ...
      it('sets the clock id', () => {
        ...
        assert.equal(log.id, 'ABC')
        assert.equal(log.clock.id, identity.publicKey)
      })
      ...
      it('sets items if given as params', async () => {
        const one = await Entry.create(ipfs, identity, 'A', 'entryA', [], new Clock('A', 0))
        const two = await Entry.create(ipfs, identity, 'A', 'entryB', [], new Clock('B', 0))
        const three = await Entry.create(ipfs, identity, 'A', 'entryC', [], new Clock('C', 0))
      ...
      it('sets heads if given as params', async () => {
        const one = await Entry.create(ipfs, identity, 'A', 'entryA')
        const two = await Entry.create(ipfs, identity, 'B', 'entryB')
        const three = await Entry.create(ipfs, identity, 'C', 'entryC')
      ...
      it('throws an error if entries is not an array', () => {
        let err
        try {
          const log = new Log(ipfs, 'A', {}, null, null, testEntryValidator)
        } catch(e) {
          err = e
        }
      ...
      it('throws an error if heads is not an array', () => {
        let err
        try {
          const log = new Log(ipfs, 'A', [], {}, null, testEntryValidator)
        } catch(e) {
          err = e
        }
        assert.notEqual(err, undefined)
      ...
      it('throws an error if validator is not defined', () => {
        let err
        try {
        ...
        assert.notEqual(err, undefined)
        assert.equal(err.message, 'Validator is required')
      })
    })
    //
    ...
    //   const expectedData = {
    //     id: 'AAA',
    //     heads: ['QmfDtxYVMLhLYTFgWmNazoCRvbHL19nayikVW4A3ccbk5o']
    //   }
    //
    //   beforeEach(async () => {
    //     log = new Log(ipfs, 'AAA', null, null, null, entryValidator, identity)
    ...
    //       heads: ['QmfDtxYVMLhLYTFgWmNazoCRvbHL19nayikVW4A3ccbk5o'],
    //       values: [
    //         'QmP5uTwAHXT1nL1sSVGgFRy38YBg5H6NrZTBy6YBJn1A9D',
    //         'QmZMF7T7x3FHBhT3A1x8uUDhsArGbcR4FTQ6VLhxgrCLLc',
    //         'QmfDtxYVMLhLYTFgWmNazoCRvbHL19nayikVW4A3ccbk5o',
    //       ]
    //     }
    //
    ...
    //       const expectedHash = 'QmUddT5cBHaAYk2bXnJw4VfcgsCDZyn3CcM39MGHuQQ6UD'
    ...
    //       const expectedData = {
    //         id: 'A',
    //         heads: ['QmXm2CKeGvpgskHD65KkegHwAGHkpoqcgCw2EnHHAVxYcU']
    //       }
    //       const expectedHash = 'QmUddT5cBHaAYk2bXnJw4VfcgsCDZyn3CcM39MGHuQQ6UD'
    //       let log = new Log(ipfs, 'A', null, null, null, entryValidator, identity)
    //       await log.append('one')
    ...
    //       const expectedData = {
    //         id: 'X',
    //         heads: ['QmUMMWChiA5B4gBAPBv9AiiB36nutZRPGkqVpktr2edPbf']
    //       }
    ...
    //       assert.equal(res.values[0].clock.id, identity.publicKey)
    ...
    //       const amount = 100
    //       let log = new Log(ipfs, 'A', null, null, null, testEntryValidator1, identity)
    //       for (let i = 0; i < amount; i ++) {
    //         await log.append(i.toString())
    //       }
    //
    ...
    //
    //     before(async () => {
    //       log = new Log(ipfs, 'A', null, null, null, testEntryValidator1, identity)
    //       await log.append("hello1")
    //     })
    //
    ...
    //
    //   beforeEach(async () => {
    //     log1 = new Log(ipfs, 'X', null, null, null, testEntryValidator1, identity)
    //     log2 = new Log(ipfs, 'X', null, null, null, testEntryValidator2, identity)
    //     log3 = new Log(ipfs, 'X', null, null, null, testEntryValidator3, identity)
    //     log4 = new Log(ipfs, 'X', null, null, null, testEntryValidator4, identity)
    //   })
    //
    //   it('joins logs', async () => {
    //     let items1 = []
    ...
    //       const n1 = await Entry.create(ipfs, identity, 'X', 'entryA' + i, [prev1])
    //       const n2 = await Entry.create(ipfs, identity, 'X', 'entryB' + i, [prev2, n1])
    //       const n3 = await Entry.create(ipfs, identity, 'X', 'entryC' + i, [prev3, n1, n2])
    //       items1.push(n1)
    //       items2.push(n2)
    //       items3.push(n3)
    //     }
    //
    //     const logA = await Log.fromEntry(ipfs, last(items2), -1, null, testEntryValidator1, identity)
    //     const logB = await Log.fromEntry(ipfs, last(items3), -1, null, testEntryValidator1, identity)
    ...
    //
    //     let log2 = await Log.fromEntry(ipfs, data.heads, 4, null, testEntryValidator, identity)
    ...
    //
    //     let log3 = await Log.fromEntry(ipfs, data.heads, 7, null, testEntryValidator, identity)
    ...
    //
    //   it('onProgress callback is fired for each entry', async () => {
    //     const log1 = new Log(ipfs, 'A', null, null, null, testEntryValidator1,identity)
    //     let items1 = []
    //     const amount = 100
    //     for(let i = 1; i <= amount; i ++) {
    //       const prev1 = last(items1)
    //       const n1 = await Entry.create(ipfs, identity, 'A', 'entryA' + i, [prev1])
    //       items1.push(n1)
    //     }
    //
    //     let i = 0
    ...
    //     }
    //
    //     const a = await Log.fromEntry(ipfs, last(items1), -1, [], testEntryValidator1, identity, callback)
    //   })
    //
    //   it('retrieves partial log from an entry hash', async () => {
    //     const log1 = new Log(ipfs, 'X', null, null, null, testEntryValidator1,identity)
    //     const log2 = new Log(ipfs, 'X', null, null, null, testEntryValidator2, identity)
    //     const log3 = new Log(ipfs, 'X', null, null, null, testEntryValidator3, identity)
    //     let items1 = []
    //     let items2 = []
    //     let items3 = []
    //     const amount = 100
    ...
    //       const n2 = await Entry.create(ipfs, log2._identityr, 'X', 'entryB' + i, [prev2, n1])
    ...
    //         const n3 = await Entry.create(ipfs, log3._identity, log3.id, 'entryC' + i, [prev3, n2], log3.clock)
    //         log1.clock.tick()
    //         log2.clock.tick()
    //         log3.clock.tick()
    ...
    //     let logA = new Log(ipfs, 'X', null, null, null, testEntryValidator1, identity)
    //     let logB = new Log(ipfs, 'X', null, null, null, testEntryValidator2, identity)
    //     let log3 = new Log(ipfs, 'X', null, null, null, testEntryValidator3, identity)
    //     let log  = new Log(ipfs, 'X', null, null, null, testEntryValidator4, identity)
    //
    //     for(let i = 1; i <= 5; i ++) {
